...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(x, MacroProcedure):
            return scheme_eval(x.apply_macro(rest, env), env)
        else:
            ops = rest.map(lambda operand: scheme_eval(operand, env))
            return scheme_apply(x, ops, env) 
        # END PROBLEM 5
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            ...
            p = Frame.lookup(self.parent, symbol)
            return p
        else:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            raise SchemeError("symbol and value lengths don't correspond")
        # END PROBLEM 11
        return child
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    lamb = LambdaProcedure(expressions.first, expressions.second, env)
    return lamb
    # END PROBLEM 9
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.second is nil:
            return scheme_eval(expressions.first, env, True)
        place = scheme_eval(expressions.first, env)
        if scheme_falsep(place): 
            return False 
        elif expressions.second is nil: 
            return place 
...
def do_define_macro(expressions, env):
    ...
    check_form(expressions, 2)
    s = expressions.first
    if isinstance(s, Pair) and scheme_symbolp(s.first):
        f = s.second
        check_formals(f)
        b = expressions.second
        env.define(s.first, MacroProcedure(f, b, env))
        return s.first
    else:
        raise SchemeError
    # END Problem 21
